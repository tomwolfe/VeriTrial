...
def test_qtc_flags():
    ...
    # delta = 200 * 100 / (1 + 100) ≈ 199.6ms → QTc ≈ 599.6ms > 500ms
    obs = [
        Observation(
    ...
    assert results[0].flag_qtc_500ms_absolute == True, "QTc > 500ms should fire flag"
...
def test_dili_hys_law():
    ...
    assert len(results) == 1
    assert results[0].hy_law_criteria_met == True, "Hy's Law criteria should be met with ALT=150, Bili=3.0"
...
def test_ctcae_grades():
    ...
    # QTc = 470ms baseline → CTCAE Grade 2
    grade_qtc = ctcae_dlt_grade(exposure=1.0, dose=1.0, max_alt=None, max_bili=None,
                                  baseline_qtc=470.0, qtc_delta=None)
    assert grade_qtc >= 2, f"Expected CTCAE Grade 2 for QTc=470, got grade {grade_qtc}"

    # QTc = 490ms baseline → CTCAE Grade 3
    grade_qtc = ctcae_dlt_grade(exposure=1.0, dose=1.0, max_alt=None, max_bili=None,
                                  baseline_qtc=490.0, qtc_delta=None)
    assert grade_qtc >= 3, f"Expected CTCAE Grade 3 for QTc=490, got grade {grade_qtc}"

    # QTc = 510ms baseline → CTCAE Grade 4
    grade_qtc = ctcae_dlt_grade(exposure=1.0, dose=1.0, max_alt=None, max_bili=None,
                                  baseline_qtc=510.0, qtc_delta=None)
    assert grade_qtc >= 4, f"Expected CTCAE Grade 4 for QTc=510, got grade {grade_qtc}"
