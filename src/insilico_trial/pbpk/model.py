...
def renal_egfr_scale(egfr_ml_min: float, fraction_renal: float) -> float:
    """Per-patient renal clearance scale (G6b).

    Single rule (ASSUMPTIONS.md §3) used by engine and validation alike:
    ``(1 - fe) + fe * min(egfr/90, 1)^0.5``. Neutral (1.0) for hepatically
    cleared drugs (fe = 0). Previously the metformin eGFR-CL correlation
    passed with NO renal mechanism (allometric confound only) - now it is
    mechanistic.
    """
    try:
        _g = max(float(egfr_ml_min), 0.0)
        _fe = min(max(float(fraction_renal), 0.0), 1.0)
    except (TypeError, ValueError):
        return 1.0
    if not onp.isfinite(_g):
        # Fail-closed: unknown kidney function must not silently clear at
        # full rate (nor NaN-poison downstream auditing into an average).
        raise ValueError(f"non-finite eGFR: {egfr_ml_min!r}")
    return (1.0 - _fe) + _fe * (min(_g / 90.0, 1.0) ** 0.5)


def qsp_params_for_drug(drug: Any) -> dict[str, float]:
    """Compound-specific QSP priors for the 9-state DILI ODE (G4).

    Single rule used by BOTH the trial engine (live ALT/GSH trajectories)
    and the post-hoc safety assessor — previously each site hardcoded the
    literature defaults (or its own copy of the override rule), so the live
    trajectories ignored compound mitochondrial toxicity entirely:
      * ``km_metabolic`` (Seahorse-derived mito IC50, mg/L) → ``IC50``
      * ``gsh_depletion_rate`` → ``k_deplete``
      * ``alt_baseline`` → ``ALT_base``
    Zero/absent fields fall back to ``_QSP_DEFAULTS`` (benchmarks unmoved).
    """
    params = dict(_QSP_DEFAULTS)
    if float(getattr(drug, "km_metabolic", 0.0) or 0.0) > 0:
        params["IC50"] = float(drug.km_metabolic)
    if float(getattr(drug, "gsh_depletion_rate", 0.0) or 0.0) > 0:
        params["k_deplete"] = float(drug.gsh_depletion_rate)
    if float(getattr(drug, "alt_baseline", 0.0) or 0.0) > 0:
        params["ALT_base"] = float(drug.alt_baseline)
    return params
...
def pbpk_ode(
    ...
    # Mechanistic CYP-mediated hepatic intrinsic clearance (G3, optional):
    # first-order extraction on unbound liver concentration. Optionality via
    # try/except-KeyError (NOT args.get("X", d)): the `.get()` string-literal
    # form breaks the bridge's textual alias expansion (gate `_expand`
    # recursively rewrites the literal `"CLint"` into nested garbage and
    # sympify crashes), while bare symbols flow cleanly through _sym_diff
    # (parameter → derivative 0), sympy, Lean binders, and numeric eval.
    # OFF defaults (CLint=0 → term exactly 0.0) keep validated benchmarks
    # byte-identical; old hand-built args dicts keep working.
    try:
        CLint = args["CLint"]  # L/h, hepatic intrinsic clearance
    except KeyError:
        CLint = 0.0
    try:
        fu_liver = args["fu_liver"]  # unbound fraction in liver
    except KeyError:
        fu_liver = 1.0
    try:
        cyp_activity = args["cyp_activity"]  # genotype/DDI scalar (>= 0)
    except KeyError:
        cyp_activity = 1.0
    ...
    # --- Liver compartment (perfusion-limited + CYP metabolism) ---
    # NOTE: the perfusion flux `dA_liver_perf` (blood↔liver exchange) is kept
    # distinct from the metabolic loss `v_met` (destroyed in the liver): the
    # central compartment reclaims only the perfusion backflow, NOT the
    # metabolized mass (folding v_met into dA_liver would credit central with
    # mass that no longer exists — caught by check_mass_conservation).
    C_liver = A_liver / V[_LIVER_IDX]
    dA_liver_perf = Q[_LIVER_IDX] * (C_p - C_liver / Kp[_LIVER_IDX])
    v_met = cyp_activity * CLint * fu_liver * C_liver
    dA_liver = dA_liver_perf - v_met
    ...
    # Reclaims the liver PERFUSION backflow only (dA_liver_perf, not dA_liver:
    # metabolized mass is gone, routed to A_elim below).
    dA_central = (
        ka * A_gut
        - dA_liver_perf
        - dA_periph
    ...
    # --- Eliminated amount accumulator (systemic + hepatic metabolism) ---
    dA_elim = CL * C_p + v_met
...
def build_pbpk_params(
    ...
    egfr_scale : float
        Renal function scale applied to clearance (>=0). Renal axis ONLY —
        hepatic impairment uses hepatic_scale (conflating them under-doses
        renally-cleared drugs in liver disease and vice versa).
    hepatic_scale : float
        Hepatic function scale (Child-Pugh B ≈ 0.6). Scales BOTH systemic CL
        and mechanistic CYP CLint — the G3/G6 interaction: liver disease
        impairs intrinsic clearance too, not just whole-body CL.
    cyp_clint : float
        Hepatic intrinsic clearance (L/h). 0.0 (default) disables the
        mechanistic pathway: validated benchmarks are byte-identical.
    fu_liver : float
        Unbound fraction in liver (0, 1].
    cyp_activity : float
        Genotype/DDI scalar on CLint (>= 0). NOTE: when the mechanistic pathway
        is enabled, apply genotype HERE *or* via genotype_scale on CL, not
        both — double-counting over-predicts clearance.

    Returns
    -------
    dict with Q, V, Kp, CL, ka, CLint, fu_liver, cyp_activity (see pbpk_ode)
    """
    ...
    # egfr_scale (renal) and hepatic_scale (hepatic) are independent axes.
    cl = max(drug.typical_cl_f * w_scaling * age_factor * genotype_scale * egfr_scale * hepatic_scale, 1e-6)
    clint = max(float(cyp_clint) * hepatic_scale, 0.0)
    # Fail-closed floors (red-team G3 finding): negative CYP activity or
    # unbound fraction RUNS METABOLISM BACKWARD (mass pumped elim→liver,
    # total conserved so no monitor can see it). Negative ka likewise
    # creates gut mass. Floor at the boundary; the ODE hot path stays
    # clamp-free per project discipline (tracer-safe, no silent alteration).
    cyp_activity = max(float(cyp_activity), 0.0)
    fu_liver = min(max(float(fu_liver), 0.0), 1.0)
    ka = max(float(drug.ka), 0.0)
    ...
    return {
        ...
        "ka": float(ka),
        "CLint": float(clint),
        "fu_liver": float(fu_liver),
        "cyp_activity": float(cyp_activity),
    }
