...
The formal gate (`check_qed_proofs`) certifies PBPK structural properties
through QED's Lean 4 agentic pipeline. The lemma set is generated by
`scripts/export_pbpk_to_qed.py` (the single source of truth, derived from
the live `pbpk/model.py` via AST analysis). What it proves and what it
does not:

| Lemma | What is proved | Method | Requires |
|---|---|---|---|
| Lemma 1: `ka * A_gut = ka * A_gut` | Gut absorption term is well-formed | `rfl` (reflexivity) | Bare Lean 4 |
...
- Numerical accuracy of the ODE solver (covered by `make validate`
  benchmarks: warfarin CL/F, moxifloxacin ΔQTc, mass-balance < 1e-7).
- Correctness across the full patient population (covered by `make test`).
- Clinical relevance of the PK outputs (covered by V&V 40 benchmarks).
- Symbolic perfusion identities over arbitrary variables (`Q*(C_p -
