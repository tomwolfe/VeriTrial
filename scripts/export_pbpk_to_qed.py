...
def build_parametric_sum_lemma(model_path: Path) -> str:
    ...
    to produce a Lean-parseable statement::
        ...
        ∀ (ka Q Kp V CL > 0), dA_gut + Q * (C_p - C_liver / Kp) + ... + CL * C_p = 0

    The positivity hypotheses (``∀ ... > 0``) are auto-generated by parsing
    the symbolic expression with QED's ``extract_positivity_hypotheses`` to
    find all variables in division denominators.
    ...
    # Parse the sum expression to find division-denominator variables and
    # auto-generate positivity hypotheses (the ∀ ... > 0 clause).
    # NOTE: The QED parser does not support ∀ syntax in raw expressions.
    # The agentic pipeline's generate_lean_code() adds the ∀ quantification
    # and positivity hypotheses automatically when it detects division
    # variables. We emit just the sum expression here.
    try:
        import sys as _sys
        _sys.path.insert(0, str(Path(__file__).resolve().parents[2] / "QED"))
        from parser import parse_equation, extract_positivity_hypotheses as _eph

        eq_node, _ = parse_equation(sum_expr)
        hyps = _eph(eq_node) if eq_node else []
    except Exception:
        hyps = []
